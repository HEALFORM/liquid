...
import { LiquidRadius, LiquidShadow } from '../../styles/theme'
import { LiquidFontSize, LiquidFontWeight } from '../../styles/theme/typography'
...
const Template: Story<IconProps & { title: string; icons: Record<string, IconType> }> = ({ title, icons, ...args }) => (
  <Flex spacing={4} direction="column">
    <Heading>
      {title}{' '}
      <Box fontWeight={LiquidFontWeight.Normal} fontSize={LiquidFontSize.Sm} color="gray.500" as="span">
    ...
    <Grid autoFit={{ min: '130px', max: '1fr' }} gap={2}>
      {Object.entries(icons)
        .slice(0, MAX_ICONS)
        .map(([moduleName, icon]) => (
          <Flex key={moduleName} borderRadius={LiquidRadius.Md} p={4} align="center" direction="column" spacing={3}>
            <Flex
            ...
            <Text truncate={16} fontSize={LiquidFontSize.Xs}>
              {moduleName}
            </Text>
          </Flex>
        ))}
